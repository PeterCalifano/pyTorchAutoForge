...
import torch
import pytest
from pyTorchAutoForge.evaluation.ModelProfiler import ModelProfiler
...
def test_profiler_with_shape(dummy_model, capsys):
    ...
    profiler = ModelProfiler(dummy_model, [2, 3])
...
def test_profiler_with_tensor(dummy_model):
    ...
    profiler = ModelProfiler(dummy_model, sample)
    prof = profiler.run_prof()
    assert profiler.last_prof is not None
...
def test_profiler_with_numpy(dummy_model):
    ...
    profiler = ModelProfiler(dummy_model, input_array)
...
def test_invalid_input(dummy_model):
    ...
    with pytest.raises(TypeError):
        ModelProfiler(dummy_model, {"invalid": "input"})
...
def test_export_trace(tmp_path, dummy_model, monkeypatch):
    ...
    profiler = ModelProfiler(dummy_model, [2, 3], output_prof_filename=outfile)
...
def test_profiler_with_numpy_dump_trace(dummy_model):
    ...
    profiler = ModelProfiler(dummy_model, input_array, output_prof_filename=outfile)
    # This should convert the numpy array to a torch tensor and run inference.
