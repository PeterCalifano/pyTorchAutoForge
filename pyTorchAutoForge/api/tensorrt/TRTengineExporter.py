...
class TRTengineExporterConfig:
    ...
    input_onnx_model: str | None = None
...
class TRTengineExporter:
    ...
    Notes:
        When a torch model is provided, intermediate ONNX export is delegated to
        `ModelHandlerONNx`. The generated ONNX filepath follows the ONNX handler
        path-resolution policy (directory mode, explicit `.onnx` file mode, and
        timestamped auto-naming when no explicit model name is provided).
    """

    def __init__(self, config: TRTengineExporterConfig) -> None:
        ...
        self.input_onnx_model = config.input_onnx_model
        self.output_engine_path = config.output_engine_path

        self.torch_model = config.torch_model
        self.input_sample = config.input_sample
        ...
        if self.torch_model is not None:
            ...
            if self.input_onnx_model is None:
                self.input_onnx_model = "./tmp_onnx_model_artifact.onnx"

            if self.input_sample is None:
                # TODO Try to create a dummy input sample reading size from model
                raise ValueError(
                    "Please provide an input sample to export a model directly from torch.nn.Module.")

            self.build_intermediate_onnx_(
                self.torch_model, self.input_sample, self.input_onnx_model)

        if self.exporter_mode == TRTengineExporterMode.PYTHON:
            # TODO Implement, see InferenceTensorRT_example.py by Umberto
            raise NotImplementedError(
                "Engine building is not implemented for Python mode yet.")
            # Define logger and builder for TensorRT
            self.logger = trt.Logger(trt.Logger.INFO)
            self.builder = trt.Builder(self.logger)

            # TODO builder.create_network( 1 << int(trt.NetworkDefinitionCreationFlag.EXPLICIT_BATCH) )
            self.network
            ...
            if shutil.which("trtexec") is None:
                raise FileNotFoundError(
                    "trtexec executable not found in system PATH. Please install TensorRT library and add it to PATH.")
        else:
            raise ValueError(
                "Invalid TRTengineExporterMode value. Please select TRTEXEC or PYTHON.")
    ...
    def build_intermediate_onnx_(self, torch_model: torch.nn.Module, input_sample: torch.Tensor, input_onnx_model: str) -> None:
        ...
        Args:
            ...
            input_sample: A sample input tensor for the model.
            input_onnx_model: Desired ONNX output path or directory, following
                `ModelHandlerONNx` export target resolution rules.

        Raises:
            FileNotFoundError: If the ONNX model was not exported successfully.
        """

        # Export intermediate ONNX model using ModelHandlerONNx API
        exported_onnx_path = ModelHandlerONNx(torch_model,
                                              dummy_input_sample=input_sample,
                                              onnx_export_path=input_onnx_model,
                                              run_export_validation=True,
                                              ).export_onnx(model_inputs=input_sample,
                                                            backend="legacy",
                                                            fallback_to_legacy=False,
                                                            )

        # Check if model was exported successfully (file exists)
        if not os.path.exists(exported_onnx_path):
            raise FileNotFoundError(
                "ONNX model was not exported successfully or incorrect path was checked. Please check any error message.")
        else:
            print("Intermediate ONNX model exported successfully to:",
                  exported_onnx_path)
            self.input_onnx_model = exported_onnx_path

    def _check_input_output_paths(self,
                                  input_onnx_model: str | None,
                                  output_engine_path: str | None) -> tuple[str, str]:
        """
        ...
        if input_onnx_model is None and self.input_onnx_model is None:
            raise ValueError(
                "Please provide an input ONNX model path as input or at helper class instantiation.")
            ...
            raise NotImplementedError(
                "Code branch is invalid or not implemented. Please report this issue to petercalifano.gs@gmail or create an issue on the GitHub repository.")

        if output_engine_path is None and self.output_engine_path is None:
            raise ValueError(
                "Please provide an output engine path as input or at helper class instantiation.")
    ...
    def _build_engine_from_onnx(self,
        ...
        input_onnx_model, output_engine_path = self._check_input_output_paths(
            input_onnx_model, output_engine_path)
        ...
        if self.exporter_mode == TRTengineExporterMode.PYTHON:
            ...
            # TODO add precision flags
            command = f"trtexec --onnx={input_onnx_model} --saveEngine={output_engine_path}"
            ...
            if result.returncode != 0:
                ...
                raise RuntimeError(
                    "Failed to build engine, trtexec returned a non-zero exit code.")
            else:
                print("trtexec output:", result.stdout.decode())
